...
export const fetchRepoList = createAsyncThunk<
  fetchResponse,
  number | undefined
>("cats/fetchRepoList", async (value = 1, { getState }) => {
  let data;
  ...
  try {
    const response = await fetch(
      `https://api.github.com/search/repositories?q=javascript&sort=${currentSortBy}&limit=20&per_page=20&page=${value}`
    );
...
const repoSlice = createSlice({
  ...
  reducers: {
    setRepo: (state, action: PayloadAction<IRepo>) => {
      const index = state.repoList.findIndex(
    ...
    removeRepo: (state, action: PayloadAction<number>) => {
      state.repoList = state.repoList.filter(
...
export const { setRepo, removeRepo, setSortBy } = actions;
